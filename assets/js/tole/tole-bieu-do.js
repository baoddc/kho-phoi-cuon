...
async function loadAllData() {
  try {
    ...
    const importWorksheet = findSheetByName(workbook, 'tole_nhap') || workbook.Sheets[workbook.SheetNames[0]];
    const exportWorksheet = findSheetByName(workbook, 'tole_xuat');
    const tonWorksheet = findSheetByName(workbook, 'tole_ton');
    const importRollsWorksheet = findSheetByName(workbook, 'tole_nhap_ct_cuon');
    const exportRollsWorksheet = findSheetByName(workbook, 'tole_xuat_ct_cuon');

    if (!importWorksheet) throw new Error("Không tìm thấy sheet Nhập (tole_nhap)");
    if (!exportWorksheet) throw new Error("Không tìm thấy sheet Xuất (tole_xuat)");
    if (!tonWorksheet) throw new Error("Không tìm thấy sheet Tồn (tole_ton)");
