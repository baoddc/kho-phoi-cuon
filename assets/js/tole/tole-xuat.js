...
const TOLE_TON_GID = '869739970';                  // gid của sheet tole_ton
...
const TOLE_TON_URL = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/export?format=xlsx&gid=${TOLE_TON_GID}`;
...
const APPS_SCRIPT_URL1 = 'https://script.google.com/macros/s/AKfycbxxvKltNqTkfWSPpNnOLeF0_HoF-kXbGxU58plBhr2eUjz-nBsh9qTWK_UqnNLw57en4w/exec';
...
let rollCount = 0;
let toleTonMaVatTuList = []; // Danh sách Mã vật tư duy nhất từ sheet tole_ton (dùng cho filter)
let toleTonKgMap = {};        // Map Mã vật tư -> số kg tồn kho (legacy, không dùng nữa)
let toleTonRows = [];         // Tất cả các dòng từ sheet tole_ton: [{maVatTu, kg, rowIndex, label}]
    ...
    if (voucherColIndex >= 0) {
      ...
      for (let i = 1; i < tableData.length; i++) {
        ...
        if (voucherValue !== undefined && voucherValue !== null) {
          ...
          // Only keep rows containing PX or DC (Xuất)
...
function openAddDataModal() {
  ...
  commonColIndices.forEach(colIdx => {
    ...
    // Column 1: Loại nhập/xuất (PX/DC dropdown for xuất)
    if (colIdx === 1) {
  ...
  // Populate roll dropdown from tole_ton
...
// Fetch tất cả dòng từ sheet tole_ton (column 4 = index 3 = Mã vật tư)
async function fetchToleTonMaVatTu() {
  if (toleTonRows.length > 0) return; // already loaded
  try {
    const resp = await fetch(TOLE_TON_URL);
    ...
    const result = [];
    // col index 3 = column 4 (Mã vật tư), skip header row (index 0)
    ...
    toleTonRows = result;
    toleTonMaVatTuList = [...new Set(result.map(r => r.maVatTu))].sort((a, b) => a.localeCompare(b, 'vi'));
  } catch (e) {
    console.error('Lỗi tải tole_ton:', e);
  }
}
...
async function populateRollDropdown() {
  const listEl = document.getElementById('rollCheckboxList');
  if (!listEl) return;
  // Show warning initially (Mã vật tư not yet filled)
  listEl.innerHTML = '<div class="text-warning fw-bold small text-center py-2">Vui lòng nhập mã vật tư</div>';
...
function filterRollDropdown() {
  ...
  const filtered = toleTonRows.filter(r => r.maVatTu.toLowerCase() === q.toLowerCase());
...
document.addEventListener('submit', async (e) => {
  try {
    ...
    if (e.target && e.target.id === 'addDataForm') {
      ...
      const cuonIdColIndex = 13; // Column N (0-indexed)
      const rowsToAdd = rollData.map(({ kg, maVatTu, cuonId }) => {
        ...
        // Ensure row is long enough for column N
