...
function addRollRow(kgValue = '', mValue = '') {
  rollCount++;
...
function updateRollTotals() {
  ...
  if (totalMEl) totalMEl.textContent = totalM.toFixed(2) + ' m';
...
function addEditRollRow(kgValue = '', mValue = '') {
  editRollCount++;
...
function updateEditRollTotals() {
  ...
  document.getElementById('editTotalKg').textContent = totalKg.toFixed(2) + ' kg';
  const editTotalMEl = document.getElementById('editTotalM');
  if (editTotalMEl) editTotalMEl.textContent = totalM.toFixed(2) + ' m';
...
document.addEventListener('submit', async (e) => {
  try {
    ...
    // ===== EDIT DATA FORM =====
      ...
      // Collect all roll kg and m values
